...
export default function PhylogeneticTree({
  ...
  useEffect(() => {
    if (!svgRef.current || !containerRef.current || !phylogeneticDataFromFile)
      return;
    ...
    if (
      ...
      Array.isArray(processedData.tree.children)
    ) {
      processedData.tree.children.forEach((branchContainer: any) => {
        if (
          branchContainer.children &&
          Array.isArray(branchContainer.children)
        ) {
    ...
    function findOrCreateChild(
      parentNode: TreeNode,
      childDataToMerge: any
    ): TreeNode {
      const childName = childDataToMerge.name;
      if (typeof childName !== "string") {
        console.warn(
          "Node without a valid name encountered during merge:",
          childDataToMerge
        );
        childDataToMerge.name =
          "Unnamed Node " + Math.random().toString(36).substr(2, 5);
      }

      if (!parentNode.children) {
        parentNode.children = [];
      }
      let foundChild = parentNode.children.find(
        (c) => c.name === childDataToMerge.name
      );
      ...
      if (!foundChild) {
        ...
        if (
          childDataToMerge.branch_length !== undefined &&
          foundChild.branch_length === undefined
        ) {
    ...
    function recursivelyMergeChildren(
      currentMergedParentNode: TreeNode,
      childrenToMergeFromJSON: any[] | undefined
    ) {
      ...
      childrenToMergeFromJSON.forEach((childJsonNode) => {
        if (typeof childJsonNode !== "object" || childJsonNode === null) {
          console.warn(
            "Invalid child node data encountered in JSON for merging:",
            childJsonNode
          );
          return;
        }
        const newOrExistingMergedChild = findOrCreateChild(
          currentMergedParentNode,
          childJsonNode
        );
        recursivelyMergeChildren(
          newOrExistingMergedChild,
          childJsonNode.children
        );
      });
    }
    ...
    processedData.tree.children.forEach((unnamedBranchContainer: any) => {
      if (
        unnamedBranchContainer.children &&
        unnamedBranchContainer.children.length > 0
      ) {
        recursivelyMergeChildren(
          mergedRootData,
          unnamedBranchContainer.children
    ...
    const updateTree = (dataForHierarchy: TreeNode) => {
      ...
      const zoomableGroup = currentSvg
        .append("g")
      ...
      const dynamicClusterHeight = Math.max(
        minViewportBasedHeight,
        calculatedTreeHeight
      );

      const baseClusterWidth = width - margin.left - margin.right;
      const dynamicClusterWidth =
        baseClusterWidth * BRANCH_LENGTH_STRETCH_FACTOR;

      const cluster = d3
        .cluster<TreeNode>()
        .size([dynamicClusterHeight, dynamicClusterWidth]);
      cluster(root);
      ...
      if (speciesNameToMatch) {
        root.each((node: d3.HierarchyNode<TreeNode>) => {
          if (
            !targetNodeForPath &&
            node.data.name &&
            node.data.name.toLowerCase() === speciesNameToMatch
          ) {
      ...
      if (targetNodeForPath !== null) {
        (targetNodeForPath as d3.HierarchyNode<TreeNode>)
          .ancestors()
          .forEach((anc) => {
      ...
      const links = zoomableGroup
        ...
        .attr("stroke", (linkDatum: d3.HierarchyLink<TreeNode>) => {
          ...
          if (
            sourceName &&
            targetName &&
            ancestorsSet.has(sourceName) &&
            ancestorsSet.has(targetName)
          ) {
      ...
      node
        ...
        .attr("text-anchor", (dHier: d3.HierarchyNode<TreeNode>) =>
          dHier.children ? "end" : "start"
        )
        ...
        zoomBehaviorRef.current = d3
          ...
          .on("zoom", (event) => {
            if (treeGroupRef.current) {
              d3.select(treeGroupRef.current).attr(
                "transform",
                event.transform
              );
            }
      ...
      const initialX =
        margin.left -
        ((dataForHierarchy.branch_length || 0) * BRANCH_LENGTH_STRETCH_FACTOR) /
          2;
    ...
    const numNodesForAnimEstimate =
      (mergedRootData.children?.length || 0) *
      (mergedRootData.children?.[0]?.children?.length || 10);
    const dynamicLoadingTimeout = Math.min(
      3000,
      ANIM_DURATION_FAST +
        (numNodesForAnimEstimate * ANIM_NODE_DELAY_FACTOR) / 10 +
        300
    );
  ...
  const handleReset = () => {
    ...
    // Reconstruct mergedRootData as in useEffect
    const processedData = JSON.parse(JSON.stringify(phylogeneticDataFromFile));
    if (typeof processedData.name === "string") {
      processedData.name = processedData.name.replace(/_/g, " ");
    }
    ...
    // Calculate dynamicClusterHeight as in useEffect
    const width = containerRef.current.clientWidth;
    const height = containerRef.current.clientHeight;
    const isSmallScreen = width < 640;
    const margin = {
    ...
    const root = d3.hierarchy<TreeNode>(mergedRootData, (d) => d.children);
    const numLeaves = root.leaves().length;
    const minViewportBasedHeight = height - margin.top * 2;
    const calculatedTreeHeight = numLeaves * DESIRED_LEAF_SEPARATION;
    const dynamicClusterHeight = Math.max(
    ...
    const initialX =
      margin.left -
      ((mergedRootData.branch_length || 0) * BRANCH_LENGTH_STRETCH_FACTOR) / 2;
    const initialY = margin.top - dynamicClusterHeight / 2;
